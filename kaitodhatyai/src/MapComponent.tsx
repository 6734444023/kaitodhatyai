...
const HATYAI_CENTER: [number, number] = [7.0000, 100.4781];
...
const getMarkerIcon = (pin: NeedPin): L.Icon | L.DivIcon => {
  ...
  return new L.Icon({
    iconUrl: 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/blue-marker.png',
    iconSize: [25, 41],
    iconAnchor: [12, 41],
    popupAnchor: [1, -34],
    shadowSize: [41, 41],
...
const ReportModal: React.FC<ReportModalProps> = ({ onClose, latLng, mode, user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(mode === 'HELP' ? 'แจ้งขอความช่วยเหลือเรียบร้อย!' : 'ปักหมุดร้านค้าเรียบร้อย!');
      onClose();
    } catch (error) {
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        <div className="modal-header">
          <h3>
            {mode === 'HELP' ? <MapPin size={24} className="mr-2 inline" /> : <Store size={24} className="mr-2 inline" />}
        ...
        <p className="text-muted text-sm mb-3">
            ตำแหน่งที่เลือก: **{latLng ? `${latLng.lat.toFixed(4)}, ${latLng.lng.toFixed(4)}` : 'กำลังระบุ...'}**
        </p>

        <form onSubmit={handleSubmit}>
          {mode === 'HELP' ? (
              <div className="form-group">
                <label htmlFor="need">สิ่งที่ต้องการ (เช่น อาหาร, ยา, ต้องการเรือ)</label>
                <textarea id="need" rows={3} value={need} onChange={(e) => setNeed(e.target.value)} required />
              </div>
          ) : (
              <div className="form-group">
          ...
          <button type="submit" className="btn btn-secondary btn-full" disabled={isSubmitting || !user}>
            {isSubmitting ? 'กำลังบันทึก...' : (mode === 'HELP' ? 'ปักหมุดแจ้งความต้องการ' : 'ปักหมุดร้านค้า')}
          </button>
        </form>
      </div>
    </div>
...
const MapComponent: React.FC<MapComponentProps> = ({ user, mode = 'HELP' }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'needs'), (snapshot) => {
      const fetchedPins: NeedPin[] = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
      } as NeedPin)); 
  ...
  const handleAcceptHelp = async (pinId: string, helperPhone: string, helperName: string) => {
    if (!helperPhone) {
  ...
  const handleToggleShopStatus = async (pinId: string, currentStatus: boolean) => {
      try {
          await updateDoc(doc(db, 'needs', pinId), {
...
interface AcceptHelpFormProps {
    pinId: string;
    onAccept: (pinId: string, helperPhone: string, helperName: string) => Promise<void>;
    user: User | null;
}

const AcceptHelpForm: React.FC<AcceptHelpFormProps> = ({ pinId, onAccept, user }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="mt-2">
            <p className="text-xs mb-1 font-semibold">ต้องการช่วยเหลือ? กรอกข้อมูล:</p>
